...
public class CancelReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
         ...
         try {
            ...
            reportType reportType = reportTypeFacade.find(reportId);

            if (reportType != null) {
                reportType.setDeletedAt(new Timestamp(System.currentTimeMillis()));
                reportTypeFacade.edit(reportType); // Ensure you have an update method in your facade to merge changes
                
                // Redirect to the same page (self-redirect) to reflect the update
                response.sendRedirect(contextPath + "/ReportView");
            } else {
            ...
            e.printStackTrace();
            request.setAttribute("errorMessage", "Error deleting report.");
            request.getRequestDispatcher("/ReportView").forward(request, response);
        }
    }
