...
public class ReportView extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
         try {
            List<reportType> reportTypeList = reportTypeFacade.findAllActiveReportType(); 
            
            request.setAttribute("reportTypes", reportTypeList); 
            
            request.getRequestDispatcher("/reportView.jsp").forward(request, response); // Forwards to the JSP page
        } catch (Exception e) {
            e.printStackTrace(); 
        }
    }
